...
import { archiveProductForCoach } from '@/services/commerce-products'
...
export async function POST(
  ...
  const result = await archiveProductForCoach(context.coachProfile, parsedProductId)
  if (result.error || !result.data) {
    return NextResponse.json({ error: result.error ?? '封存商品失敗。' }, { status: 400 })
  }
