...
export async function POST(_request: Request, { params }: { params: Promise<{ enrollmentId: string }> }) {
  ...
  const result = await transitionTeamEnrollmentForCoach(context.coachProfile, parsedEnrollmentId, 'end')
  if (result.error || !result.data) return NextResponse.json({ error: result.error ?? '結束 enrollment 失敗。' }, { status: 400 })
