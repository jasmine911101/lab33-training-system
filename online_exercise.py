...
alter table public.athlete_events add column if not exists title text;
alter table public.athlete_events add column if not exists event_type text;
alter table public.athlete_events add column if not exists start_date date;
alter table public.athlete_events add column if not exists end_date date;
alter table public.athlete_events add column if not exists notes text;
""".strip()
...
def fetch_athlete_events(athlete_id):
    try:
        data = (
            write_client().table("athlete_events")
...
def create_athlete_event(athlete_id, title, event_type, start_date, end_date, notes):
    normalized_title = clean_manual_cell(title) or clean_manual_cell(event_type) or "一般事件"
    return (
        write_client().table("athlete_events")
        .insert(
            {
                ...
                "event_type": clean_manual_cell(event_type) or "其他",
                "start_date": date_to_iso(start_date),
                "end_date": date_to_iso(end_date),
                "notes": clean_manual_cell(notes),
            }
        )
        .execute()
    )
...
def update_athlete_event(event_id, title, event_type, start_date, end_date, notes):
    normalized_title = clean_manual_cell(title) or clean_manual_cell(event_type) or "一般事件"
    return (
        write_client().table("athlete_events")
        .update(
        ...
        .eq("id", to_plain_int(event_id))
        .execute()
    )


def delete_athlete_event(event_id):
    return (
        write_client().table("athlete_events")
        .delete()
        .eq("id", to_plain_int(event_id))
...
def render_event_setup_help(error):
    render_sql_setup_help(error, "讀取或新增一般事件", ATHLETE_EVENT_SETUP_SQL)
...
def render_athlete_program_section(selected_athlete):
    ...
    athlete_events_df, athlete_events_error = fetch_athlete_events(selected_athlete["id"])
    if athlete_events_error:
        render_event_setup_help(athlete_events_error)
        return

    athlete_blocks_df = annotate_assignment_report_status(athlete_blocks_df)

    render_week_calendar_assignment(selected_athlete, blocks_df, athlete_blocks_df, athlete_events_df)

    if athlete_blocks_df.empty and athlete_events_df.empty:
        st.info("這位學員目前還沒有安排任何課表或一般事件。")
    elif athlete_blocks_df.empty:
...
def render_week_calendar_assignment(selected_athlete, blocks_df, athlete_blocks_df, athlete_events_df):
    with st.container(border=True):
        st.subheader("加入板塊 / 新增事件")

        block_options = blocks_df.to_dict("records") if blocks_df is not None and not blocks_df.empty else []
        block_by_id = {
            to_plain_int(block["id"]): block
            for block in block_options
            if to_plain_int(block.get("id")) is not None
        }
        ...
        if not selected_calendar_range:
            st.info("請先在上方行事曆點選一天，或拖曳選取日期區間，再安排課表或新增一般事件。")
            return
        ...
        st.markdown(f"#### 安排 {date_text}")

        if blocks_df.empty:
            st.info("目前沒有任何 block。你仍然可以在下方新增一般事件。")
        else:
        ...
        with st.form(f"create_general_event_form_{selected_athlete['id']}", clear_on_submit=True):
            st.markdown("##### 新增一般事件")
            event_title = st.text_input(
                "事件名稱",
                placeholder="例如：季前檢測、友誼賽、體能測驗",
                key=f"general_event_title_{selected_athlete['id']}",
            )
            event_type = st.selectbox(
                "事件類型",
                GENERAL_EVENT_TYPES,
                key=f"general_event_type_{selected_athlete['id']}",
            )
            event_date_col1, event_date_col2 = st.columns(2)
            event_start_date = event_date_col1.date_input(
                "開始日期",
                value=start_date,
                key=f"general_event_start_{selected_athlete['id']}",
            )
            event_end_date = event_date_col2.date_input(
                "結束日期",
                value=end_date,
                key=f"general_event_end_{selected_athlete['id']}",
            )
            event_notes = st.text_area(
                "備註",
                height=70,
                key=f"general_event_notes_{selected_athlete['id']}",
            )
            event_submitted = st.form_submit_button("新增一般事件", use_container_width=True)
...
def calendar_item_identifier(kind, row_id):
    normalized_id = to_plain_int(row_id)
    if normalized_id is None:
        return ""
    return f"{kind}:{normalized_id}"


def parse_calendar_item_identifier(value):
    raw_value = str(value or "").strip()
    if not raw_value:
        return "", None
    if ":" in raw_value:
        kind, raw_id = raw_value.split(":", 1)
        return kind, to_plain_int(raw_id)
    return "assignment", to_plain_int(raw_value)


def athlete_event_title(row):
    if not isinstance(row, dict):
        return "一般事件"
    return clean_manual_cell(row.get("title")) or clean_manual_cell(row.get("event_type")) or "一般事件"


def general_event_color(event_type):
    return GENERAL_EVENT_TYPE_COLORS.get(clean_manual_cell(event_type), "#2563EB")


def render_delete_athlete_event_confirmation(event_id, event_title):
    with st.container(border=True):
        st.warning(f"確認要刪除一般事件 {event_title} 嗎？")
        col1, col2 = st.columns(2)
        if col1.button("確認刪除", key=f"confirm_delete_event_{event_id}", use_container_width=True):
            try:
                delete_athlete_event(event_id)
                st.session_state.pop("pending_delete_event_id", None)
                event_item_id = calendar_item_identifier("event", event_id)
                ...
                render_event_setup_help(exc)
        if col2.button("取消", key=f"cancel_delete_event_{event_id}", use_container_width=True):
            st.session_state.pop("pending_delete_event_id", None)
            st.rerun()


def render_athlete_event_edit_form(row, form_key_prefix="calendar_edit_athlete_event"):
    event_id = to_plain_int(row.get("id"))
    current_type = row.get("event_type") or GENERAL_EVENT_TYPES[0]
    event_type_options = GENERAL_EVENT_TYPES if current_type in GENERAL_EVENT_TYPES else [current_type] + GENERAL_EVENT_TYPES
    start_value = row.get("start_date")
    end_value = row.get("end_date") if has_value(row.get("end_date")) else start_value

    with st.form(f"{form_key_prefix}_{event_id}"):
        title = st.text_input(
            "事件名稱",
            value=row.get("title") or "",
            placeholder="輸入事件名稱",
            key=f"{form_key_prefix}_title_{event_id}",
        )
        event_type = st.selectbox(
            "事件類型",
            event_type_options,
            index=event_type_options.index(current_type),
            key=f"{form_key_prefix}_type_{event_id}",
        )
        date_range = st.date_input(
            "日期區間",
            value=(parse_date_value(start_value), parse_date_value(end_value)),
            key=f"{form_key_prefix}_date_{event_id}",
        )
        notes = st.text_area(
            "備註",
            value=row.get("notes") or "",
            key=f"{form_key_prefix}_notes_{event_id}",
        )
        submitted = st.form_submit_button("儲存這個一般事件", use_container_width=True)

    if submitted:
        try:
            start_date, end_date = normalize_date_range(date_range)
            update_athlete_event(event_id, title, event_type, start_date, end_date, notes)
            st.success("已更新一般事件。")
            st.rerun()
        except Exception as exc:
            render_event_setup_help(exc)

    if st.button("刪除這個一般事件", key=f"{form_key_prefix}_ask_delete_{event_id}", use_container_width=True):
        st.session_state["pending_delete_event_id"] = event_id

    if st.session_state.get("pending_delete_event_id") == event_id:
        render_delete_athlete_event_confirmation(event_id, athlete_event_title(row))


def render_general_event_detail_card(event_id, athlete_events_df, editable=False):
    if athlete_events_df is None or athlete_events_df.empty or "id" not in athlete_events_df.columns:
        return

    event_rows = athlete_events_df[
        athlete_events_df["id"].astype(str) == str(event_id)
    ]
    if event_rows.empty:
        return

    row = event_rows.iloc[0].to_dict()
    title = athlete_event_title(row)
    date_text = date_range_label(row)
    event_type = row.get("event_type") or "一般事件"

    with st.container(border=True):
        st.markdown(f"### {title}")
        meta_items = [event_type]
        if has_value(date_text):
            meta_items.append(date_text)
        st.caption("｜".join([str(item) for item in meta_items if has_value(item)]))
        if has_value(row.get("notes")):
            st.info(f"備註：{row.get('notes')}")
        if editable:
            render_athlete_event_edit_form(row)


def render_calendar_item_detail_card(
    selected_item_id,
    athlete_blocks_df,
    athlete_events_df,
    block_by_id,
    ...
    item_kind, item_id = parse_calendar_item_identifier(selected_item_id)
    if item_kind == "event":
        render_general_event_detail_card(item_id, athlete_events_df, editable=editable)
        return
...
def render_assignment_calendar(athlete_id, athlete_blocks_df, athlete_events_df, block_by_id, editable_detail=False):
    st.markdown("#### 課表行事曆")
    ...
    if st_calendar is None:
        ...
        if calendar_range and calendar_range[0] == calendar_range[1]:
            ...
            st.success(f"已選取 {selected_date_text}，可以直接在下方安排這一天的課表或事件。")
        ...
        render_frontend_schedule_calendar(
            ...
            selected_assignment_id=selected_event_id,
        )
    ...
    if selected_event_id:
        render_calendar_item_detail_card(
            selected_event_id,
            athlete_blocks_df,
            athlete_events_df,
            block_by_id,
...
@st.fragment
def render_student_schedule_calendar(athlete_id, athlete_blocks_df, athlete_events_df, blocks_df):
    st.markdown("#### 課表行事曆")
    ...
    valid_assignment_ids = {
        calendar_item_identifier("assignment", row.get("id"))
        for row in athlete_blocks_df.to_dict("records")
        if to_plain_int(row.get("id")) is not None
    }
    valid_assignment_ids.update(
        {
            calendar_item_identifier("event", row.get("id"))
            for row in athlete_events_df.to_dict("records")
            if to_plain_int(row.get("id")) is not None
        }
    )
    ...
    if not events:
        st.info("目前沒有可顯示在行事曆上的安排。")
        return

    details = build_student_calendar_component_details(athlete_blocks_df, athlete_events_df, block_by_id)
    ...
    if action_type == "save_report":
        item_kind, assignment_id = parse_calendar_item_identifier(action.get("assignment_id"))
        if item_kind != "assignment" or not assignment_id:
            return
        for row in action.get("rows") or []:
            update_athlete_block_exercise_row(row.get("id"), row)
        st.session_state[selected_event_key] = calendar_item_identifier("assignment", assignment_id)
...
def render_frontend_schedule_calendar(
    ...
    payload = {
        "events": events,
        "details": build_assignment_frontend_details(athlete_blocks_df, athlete_events_df, block_by_id),
      ...
      (() => {{
        ...
        function renderList() {{
          ...
          if (!sortedEvents.length) {{
            listNode.innerHTML = `<div class="lab33-empty">目前沒有安排。</div>`;
            return;
          }}
        ...
        function renderSections(sections, emptyMessage) {{
          if (!sections.length) return `<div class="lab33-empty">${{escapeHtml(emptyMessage || "這個板塊目前沒有詳細動作內容。")}}</div>`;
...
def build_assignment_frontend_details(athlete_blocks_df, athlete_events_df, block_by_id):
    details = {}

    if athlete_blocks_df is not None and not athlete_blocks_df.empty:
        for row in athlete_blocks_df.to_dict("records"):
            assignment_id = calendar_item_identifier("assignment", row.get("id"))
    ...
    if athlete_events_df is not None and not athlete_events_df.empty:
        for row in athlete_events_df.to_dict("records"):
            event_id = calendar_item_identifier("event", row.get("id"))
            title = athlete_event_title(row)
            event_type = safe_frontend_text(row.get("event_type") or "一般事件")
            details[event_id] = {
                "kind": "general_event",
                "block_label": title,
                "meta": "｜".join([
                    item
                    for item in [event_type, safe_frontend_text(date_range_label(row))]
                    if item
                ]),
                "event_name": title,
                "date_range": safe_frontend_text(date_range_label(row)),
                ...
                "goal": "",
                "training_element": "",
                "description": safe_frontend_text(row.get("notes")),
                "sections": [],
...
def build_student_calendar_component_details(athlete_blocks_df, athlete_events_df, block_by_id):
    details = {}
...
def build_assignment_calendar_events(athlete_blocks_df, athlete_events_df, block_by_id, selected_assignment_id=""):
    ...
    if athlete_blocks_df is not None and not athlete_blocks_df.empty:
        for row in athlete_blocks_df.to_dict("records"):
            ...
            item_id = calendar_item_identifier("assignment", row.get("id"))
            ...
            is_selected = item_id == str(selected_assignment_id)
            event = {
                "id": item_id,
                "title": title,
                "start": start,
    ...
    if athlete_events_df is not None and not athlete_events_df.empty:
        for row in athlete_events_df.to_dict("records"):
            start = calendar_date_string(row.get("start_date"))
            if not start:
                continue
            event_type = row.get("event_type") or "一般事件"
            title = athlete_event_title(row)
            item_id = calendar_item_identifier("event", row.get("id"))
            event_color = general_event_color(event_type)
            is_selected = item_id == str(selected_assignment_id)
            display_title = title if title == event_type else f"{event_type}｜{title}"
            event = {
                "id": item_id,
                "title": display_title,
                "start": start,
    ...
    return sorted(events, key=lambda row: (str(row.get("start") or ""), str(row.get("title") or "")))
...
def student_page():
    ...
    if athlete_blocks_df.empty and athlete_events_df.empty:
        st.info("目前還沒有被安排任何課表或一般事件。")
        st.caption(
            f"目前登入帳號對應到 athletes.id = {selected_athlete.get('id')}。"
            "請確認 Supabase 的 athlete_blocks.athlete_id 或 athlete_events.athlete_id 是否等於這個 ID。"
        )
    else:
